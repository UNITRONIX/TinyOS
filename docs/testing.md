...
The smoke test captures serial output through QEMU's serial file backend in `build/boot-smoke.log` and passes when QEMU keeps running until the timeout and the boot, boot-module validation, boot-module address-space tracking, ELF validation, RAMFS file tools, syscall validation, device registry, RAM block device, block VFS mount, framebuffer surface, device RAMFS metadata, address-space, address-space protection flag, paging, paging protection flag, PIT IRQ0, keyboard IRQ1, task stack ownership, context ABI and scheduler tick markers appear.
