...
namespace
{
    ...
    void set_gate(int vector, void (*handler)())
    {
        ...
        g_idt[vector].zero = 0;
        g_idt[vector].type_attributes = InterruptGate;
...
namespace tinyos::arch::interrupts
{
    void initialize()
    {
        ...
        {
            set_gate(vector, reinterpret_cast<void (*)()>(isr_stub_table[vector]));
        }

        for (int irq = 0; irq < 16; ++irq)
        {
            set_gate(32 + irq, reinterpret_cast<void (*)()>(irq_stub_table[irq]));
        }
